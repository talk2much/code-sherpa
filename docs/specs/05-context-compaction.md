...
三种策略各有适用场景，选择逻辑由 `file_read` 调用方控制：

```
读文件
  │
  ├── 默认路径（Tool handler 自动选）
  │     └── extract_signatures
  │         适用: 不知道文件里面有什么，先看骨架
  │         效果: 返回所有导出符号的签名，砍掉实现体
  │         比如: 第一次打开 order.service.ts，返回 createOrder()、processRefund() 等 5 个函数签名
  │
  ├── Agent 显式指定（LLM 判断需要更多信息）
  │     └── summarize
  │         适用: 看骨架不够，需要理解逻辑但不想读几百行实现
  │         代价: 多一次 LLM 调用，耗时增加 1-3 秒
  │         效果: 返回 3-5 句自然语言摘要
  │         比如: "这个文件是订单服务的核心，包含创建订单（对接支付网关）、退款（含分账回滚）、
  │                 批量操作三个主流程，使用了 Saga 模式处理分布式事务"
  │
  └── 精确查询后（已知目标符号）
        └── show_callsite
            适用: ast_query 已经定位到具体函数，只看它的调用关系
            效果: 只保留目标函数的调用点，其余代码全部省略
            比如: 查 createOrder 的依赖 → 只显示 createOrder 调用 prisma.order.create 的那一行上下文
```

| 策略 | 谁选 | 什么时候 | 输出大小 | 耗时 |
|------|:---:|------|------|:---:|
| `extract_signatures` | 程序（默认） | 文件超阈值，首次打开 | 几十行骨架 | 0ms |
| `summarize` | LLM（显式） | 骨架不够，要理解逻辑 | 3-5 句话 | 1-3s |
| `show_callsite` | 程序（上下文） | 已知目标符号，看调用点 | 几行上下文 | 0ms |
```
